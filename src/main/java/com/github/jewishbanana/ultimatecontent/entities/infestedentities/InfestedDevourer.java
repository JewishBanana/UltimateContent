			...
			public void run() {
				...
				if (inWallTicks > 0) {
					...
					if (inWallTicks == 1) {
						...
						if (target != null && (!target.isValid() || !Utils.isLocationsWithinDistance(entityLoc, target.getLocation(), 625))) {
							...
							Block spot = BlockUtils.rayTraceForBlock(targetLoc, Utils.getRandomizedVector(1f, 0, 1f).setY(random.nextFloat() / 2 - 0.7), 9.0, temp -> !temp.isPassable());
							if (spot != null) {
				...
				if (wallCreepCooldown == 0) {
					...
					if (isTargetInRange(entity, 9, 625, false) || (!blocks.isEmpty() && !Utils.isLocationsWithinDistance(entityLoc, BlockUtils.getCenterOfBlock(blocks.get(0)), 2.25f)))
						for (int i=0; i < 5; i++) {
							Block b = BlockUtils.rayTraceForBlock(entityLoc.add(0, .5, 0), Utils.getRandomizedVector(1f, 0, 1f).setY(random.nextFloat() / 2 - 0.7), 8.0, block -> !block.isPassable());
							if (b != null) {
