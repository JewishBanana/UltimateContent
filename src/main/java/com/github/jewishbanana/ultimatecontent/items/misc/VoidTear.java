...
public class VoidTear extends BaseItem {
	...
	public static String REGISTERED_KEY = "ui:void_tear";
		...
		new RepeatingTask(0, 5) {
			...
			public void run() {
				...
				for (Entity e : world.getNearbyEntities(loc, .5, .5, .5))
					if (e instanceof Player && !Utils.isEntityImmunePlayer(e))
						Utils.pureDamageEntity((LivingEntity) e, 1.0, "deaths.unstableRift", false, DamageCause.VOID);
				if (var[0] > 0)
					var[0]-=5;
				else {
					var[0] = 60;
					...
					if (var[1] >= 6 && com.github.jewishbanana.deadlydisasters.entities.CustomEntityType.VOIDGUARDIAN.canSpawn()) {
						entity = (Mob) loc.getWorld().spawnEntity(loc, EntityType.ZOMBIE);
						ce = new com.github.jewishbanana.deadlydisasters.entities.endstormentities.VoidGuardian(entity, com.github.jewishbanana.deadlydisasters.Main.getInstance(), random);
						...
						ce = new com.github.jewishbanana.deadlydisasters.entities.endstormentities.VoidStalker(entity, com.github.jewishbanana.deadlydisasters.Main.getInstance(), random);
					} else if (var[1] == 3 && com.github.jewishbanana.deadlydisasters.entities.CustomEntityType.ENDWORM.canSpawn()) {
						entity = (Mob) loc.getWorld().spawnEntity(loc, EntityType.ZOMBIE);
						ce = new com.github.jewishbanana.deadlydisasters.entities.endstormentities.EndWorm(entity, com.github.jewishbanana.deadlydisasters.Main.getInstance(), random);
						...
						ce = new com.github.jewishbanana.deadlydisasters.entities.endstormentities.EndTotem(entity, com.github.jewishbanana.deadlydisasters.Main.getInstance(), random);
...
//						ce = new deadlydisasters.entities.endstormentities.BabyEndTotem(entity, deadlydisasters.Main.getInstance(), random);
	...
	public ItemBuilder createItem() {
		return ItemBuilder.create(getType(), Material.GHAST_TEAR).setHiddenEnchanted(VersionUtils.getUnbreaking()).assembleLore().build();
	}
	public static void register() {
		ItemType.registerItem(REGISTERED_KEY, VoidTear.class);
	}
}
