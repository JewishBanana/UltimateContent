...
public class SpawnPlatform extends AbilityAttributes {
	...
	private static final Queue<SpawnPlatform> list = new ArrayDeque<>();
	...
	public void activate(Location loc, GenericItem base) {
		...
		plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
			...
			plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
				playSound(loc, Sound.BLOCK_GLASS_BREAK, .5, .75);
				...
				plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
					playSound(loc, Sound.BLOCK_GLASS_BREAK, .5, .5);
