...
public class VersionUtils {
	...
	static {
		...
		if (isMCVersionOrAbove("1.18"))
			...
			desertBiomes = new HashSet<>(Arrays.asList(Biome.DESERT, Biome.BADLANDS, Biome.ERODED_BADLANDS,
					getConstant(Biome.class, "WOODED_BADLANDS_PLATEAU"), getConstant(Biome.class, "MODIFIED_WOODED_BADLANDS_PLATEAU"),
					getConstant(Biome.class, "MODIFIED_BADLANDS_PLATEAU"), getConstant(Biome.class, "DESERT_LAKES"),
					getConstant(Biome.class, "DESERT_HILLS"), getConstant(Biome.class, "BADLANDS_PLATEAU")));
	...
	private static <T> T getConstant(Class<T> type, String name) {
		try {
			// Biome is an enum on legacy servers and a registry-backed interface on current servers. Reflection keeps the
			// generated bytecode compatible with both forms.
			return type.cast(type.getField(name).get(null));
		} catch (ReflectiveOperationException | ClassCastException exception) {
			throw new IllegalArgumentException("No " + type.getSimpleName() + " constant named " + name, exception);
		}
	}
